...
function CreateDictionaries() {
  ...
  foreach ($info['results'] as $i => $entry) {
    if (!isset($entry['dict'])) {
      ...
      $count = count($info['results']);
      $index = $i + 1;
      $status = "Created $index of $count dictionaries...";
      break;
    }
  }
}
      ...
      if (is_file($body) && is_file($dict)) {
        ...
        for ($level = 1; $level <= 11; $level++) {
          ...
          if (exec("brotli -q $level -D $dict -o $tmp $body", $output, $result) === false || $result !== 0) {
            $error = "Error sbr compressing body";
            break 2;
          }
          $comp['sbr']["$level"] = filesize($tmp);
          if (is_file($tmp)) {unlink($tmp);}
        }
        $info['results'][$i]['comp'] = $comp;
        $count = count($info['results']);
        $index = $i + 1;
        $status = "Tested compression on $index of $count pages...";
      } else {
